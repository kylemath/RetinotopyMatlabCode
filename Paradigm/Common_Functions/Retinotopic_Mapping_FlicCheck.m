...
Fix_Cross_Targ = cross_matrix(16) * 255;
Fix_Cross_Targ(:,:,2) = Fix_Cross_Targ;
FixCrossTargTexture = Screen('MakeTexture', Win, Fix_Cross_Targ);
...
%     %Trigger BOXY DOIL2
%     outp(address_eros,128);  
%     outp(address_eeg,128);  
%     WaitSecs(.05);
%     outp(address_eros,0);  
%     outp(address_eeg,0);
%     
...
while CurrTime < CyclingEnd    
    ...
    elseif strcmpi(Parameters.Direction, '-')
        ...
        %%  % Save Current frame time & condition
    ...
    if sum(CurrEvents > 0 & CurrEvents < Parameters.Event_Duration)
        ...
        %turn the fixation white
        Screen('DrawTexture', Win, FixCrossTargTexture);  
...
%             %Trigger Response BOxy aux and EEG marker
%             outp(address_eros,12);  %TMS is only the 8th pin
%             outp(address_eeg,12);  %TMS is only the 8th pin
%             WaitSecs(.05);
...
%% Farewell screen
Screen('FillRect', Win, Parameters.Background, Rect);
DrawFormattedText(Win, 'Goodbye & Thank you very much!', 'center', 'center', Parameters.Foreground); 
Screen('Flip', Win);
WaitSecs(Parameters.TR * Parameters.Overrun);
ShowCursor;
Screen('CloseAll');
